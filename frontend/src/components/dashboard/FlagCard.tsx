...
export function FlagCard({ flag, onSelectDetails }: FlagCardProps) {
  ...
  let bgClass = "bg-blue-50/70 border-blue-200 text-blue-900";
    ...
    <div className={`rounded-2xl border p-5 shadow-2xs transition-all ${bgClass}`}>
      <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
          ...
          {flag.member_name && (
            <span className="text-xs font-medium text-stone-500 bg-stone-100 px-2 py-0.5 rounded-md border border-stone-200">
      ...
      {/* Evidence Clause */}
      {refs && (
        <div className="mt-3 flex items-center gap-1.5 text-xs font-mono text-stone-600 bg-white/70 rounded-lg p-2 border border-stone-200/80">
      ...
      {/* Action buttons */}
      <div className="mt-4 flex items-center gap-3 pt-3 border-t border-stone-200/60">
        <button
          type="button"
          onClick={onSelectDetails}
          className="rounded-xl border border-stone-300 bg-white px-3.5 py-1.5 text-xs font-semibold text-stone-800 hover:bg-stone-50 transition-colors shadow-2xs"
        >
          See Details
        </button>
